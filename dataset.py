...
class RegressionDataset(Dataset):
    ...
    def _load_dataset(self, data_dirpath, task):
        ...
        self.train, self.valid, self.train_scores, self.valid_scores = train_test_split(
            x, y, test_size=0.1, random_state=self.rng
        )

    def sample(self, n):
        indices = np.random.randint(0, len(self.train), n)
        return (
            [self.train[i] for i in indices],
    ...
    def weighted_sample(self, n, rank_coefficient=0.01):
        ...
        indices = list(
            torch.utils.data.WeightedRandomSampler(
                weights=weights.tolist(), num_samples=n, replacement=True
            )
        )
    ...
    def add(self, batch):
        ...
        train_seq, val_seq, train, val = train_test_split(
            np.array(samples), scores, test_size=0.1, random_state=self.rng
        )
        self.train_scores = np.concatenate((self.train_scores, train), axis=0).reshape(
            -1
        )
    ...
    def top_k(self, k):
        data = (
            np.concatenate((self.train, self.valid), axis=0),
    ...
    def top_k_collected(self, k):
        ...
        seqs = np.concatenate(
            (self.train[self.train_added :], self.valid[self.val_added :]), axis=0
        )
        data = (seqs, scores)
        return self._top_k(data, k)
