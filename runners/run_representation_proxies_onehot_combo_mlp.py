...
ONEHOT_COMBO_MLP_PROBE_SPECS: tuple[ProbeSpec, ...] = (
    ProbeSpec(
        name="one_hot_mlp",
        feature_name="one_hot_flat",
        estimator_name="mlp",
        search_grid=MLP_ARCHITECTURES,
        description="AA one-hot only with MLP regression.",
    ),
    ProbeSpec(
        name="one_hot_pca_mlp",
        feature_name="one_hot_flat",
        estimator_name="mlp",
        search_grid=MLP_ARCHITECTURES,
        pca_components=128,
        description="AA one-hot only with PCA + MLP regression.",
    ),
    ProbeSpec(
        name="mean_pool_plus_one_hot_mlp",
        feature_name="mean_pool_plus_one_hot",
        estimator_name="mlp",
        search_grid=MLP_ARCHITECTURES,
        description="Raw mean-pooled ESM + AA one-hot with MLP regression.",
    ),
    ProbeSpec(
        name="mean_pool_l2_plus_one_hot_mlp",
        feature_name="mean_pool_l2_plus_one_hot",
        estimator_name="mlp",
        search_grid=MLP_ARCHITECTURES,
        description="L2-normalized mean-pooled ESM + AA one-hot with MLP regression.",
    ),
    ProbeSpec(
        name="mean_pool_zscore_plus_one_hot_mlp",
        feature_name="mean_pool_zscore_plus_one_hot",
        estimator_name="mlp",
        search_grid=MLP_ARCHITECTURES,
        description="Z-scored mean-pooled ESM + AA one-hot with MLP regression.",
    ),
)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Benchmark one-hot and mean-pooled+one-hot representation proxies with MLP."
    )
...
def main() -> None:
    ...
    runner = BenchmarkRunner(
        ...
        probe_specs=ONEHOT_COMBO_MLP_PROBE_SPECS,
    ...
    summary = runner.run()
    logger.info(
        "Finished onehot+combo MLP benchmark for %d tasks. Results in %s",
        len(summary["tasks"]),
        args.output_dir,
    )
